...
void AutoSplatoon::handleSerialStatus(uint8_t status)
{
    if (status == CONNECTING) {
        ...
        ui->serialConnectButton->setText("Abort");
        ...
        ui->serialStatusLabel->setText("Failed");
    } else {
...
void AutoSplatoon::on_flashButton_clicked()
{
    ...
    ui->serialStatusLabel->setText("Flashing");

    QElapsedTimer timer;
    timer.start();
    QString appDir = QCoreApplication::applicationDirPath();
    QString cmd = appDir + "/esptool";
    qDebug() << cmd;
    QStringList arg;
...
//    arg << "0x8000";arg << QApplication::applicationDirPath()+"/partition-table.bin";
    ...
    if(flag == -1)
        ui->serialStatusLabel->setText("Failure");
    else
        ui->serialStatusLabel->setText("Success");
    ui->serialConnectButton->setEnabled(true);
    ui->flashButton->setEnabled(true);
    ui->serialPortsBox->setEnabled(true);
    ui->serialRefreshButton->setEnabled(true);
...
void AutoSplatoon::executeTask()
{
    ...
    manControl2->sendCommand("Sel", interval);
    //startFlag = false;
...
//    ui->label_4->setEnabled(false);
    ui->label_2->setText("Row:");
    ui->label_4->setText("Col:");
